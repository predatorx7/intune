...
public class SwiftMsalFlutterPlugin: NSObject, FlutterPlugin {
    ...
    private func loadAccounts(result: @escaping FlutterResult) {
        applicationContext!.accountsFromDevice(for: MSALAccountEnumerationParameters(), completionBlock: { accounts, error in
            if error != nil {
    ...
    private func acquireToken(scopes: [String], result: @escaping FlutterResult, locale: String?, clearSession: Bool?) {
        ...
        let parameters = MSALInteractiveTokenParameters(scopes: scopes, webviewParameters: webViewParamaters!)
    ...
    private func acquireTokenSilent(scopes: [String], result: @escaping FlutterResult) {
        ...
        let silentParameters = MSALSilentTokenParameters(scopes: scopes, account: currentAccount!)
        applicationContext!.acquireTokenSilent(with: silentParameters, completionBlock: { msalresult, error in
    ...
    private func initialize(clientId: String, authority: String, result: @escaping FlutterResult, redirectUri: String, keychain: String?, privateSession: Bool) {
        ...
        do {
            try initMSAL(result: result, privateSession: privateSession)

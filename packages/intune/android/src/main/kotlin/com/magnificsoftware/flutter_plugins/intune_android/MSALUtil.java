...
package com.hdfcbank.ionicframework.intune;
...
public final class MSALUtil {
   ...
   *
   * @param fromActivity the Activity from which the auth request is made.
   * @param scopes Scopes for the requested token.
   * @param loginHint a prompt for the login dialog, can be null if unused.
   * @param callback callback to receive the result of the auth attempt.
   * @throws MsalException MSAL error occurred.
   ...
   *
   * @param appContext A Context used to initialize the MSAL context, if needed.
   * @param aadId Id of the user.
   * @param scopes Scopes for the requested token.
   * @param callback callback to receive the result of the auth attempt.
   * @param forceRefresh force refreshing the token.
   * @throws MsalException MSAL error occurred.
   * @throws InterruptedException Thread was interrupted.
   */
  @WorkerThread
  public static void acquireTokenSilent(
    ...
    if (account == null) {
      ...
      callback.onError(
          new MsalUiRequiredException(
              MsalUiRequiredException.NO_ACCOUNT_FOUND, "no account found for " + aadId));
      return;
    }
   ...
   * @param aadId Id of the user.
   * @param scopes Scopes to request the token for.
   * @return the authentication result, or null if it fails.
   * @throws MsalException MSAL error occurred.
   * @throws InterruptedException Thread was interrupted.
   */
  @WorkerThread
  public static IAuthenticationResult acquireTokenSilentSync(
      @NonNull final Context appContext,
      @NonNull final String aadId,
      @NonNull final String[] scopes)
      throws MsalException, InterruptedException {
   ...
   * @throws InterruptedException Thread was interrupted.
   */
  public static void signOutAccount(@NonNull final Context appContext, @NonNull final String aadId)
      throws MsalException, InterruptedException {
    initializeMsalClientApplication(appContext);
    final IAccount account = getAccount(aadId);
    ...
    if (mMsalClientApplication instanceof IMultipleAccountPublicClientApplication) {
      ...
      if (singleAccountPCA != null) {
        ...
        if (accountResult != null) {
          ...
          if (account != null && !account.getId().equals(aadId)) account = null;
        }
      }
    }
    return account;
  ...
  private static synchronized void initializeMsalClientApplication(final Context appContext)
    ...
    if (mMsalClientApplication == null) {
      ...
      int authConfig =
          appContext
              .getResources()
              .getIdentifier("auth_config", "raw", appContext.getPackageName());

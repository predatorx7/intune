...
public class IntunePlugin extends Plugin {
  ...
  public void load() {
    ...
    final SharedPreferences prefs =
        getContext().getSharedPreferences(SETTINGS_PATH, Context.MODE_PRIVATE);
    ...
    Thread thread =
        new Thread(
            () -> {
              ...
              try {
                if (interactive) {
                  MSALUtil.acquireToken(
                      getActivity(), scopes, null, forcePrompt, new AuthCallback());
                } else {
                  MSALUtil.acquireTokenSilent(
                      getActivity(), upn, scopes, new AuthCallback(), forceRefresh);
                }
        ...
        new Thread(
            () -> {
              ...
              try {
                ...
                MSALUtil.acquireToken(
                    getActivity(), MSAL_SCOPES, loginHint, forcePrompt, new AuthCallback());
              } catch (MsalException | InterruptedException e) {
                Logger.error("Authentication exception occurred", e);
                showMessage("Authentication exception occurred - check logcat for more details.");
              }
  ...
  private class AuthCallback implements AuthenticationCallback {
    ...
    public void onError(final MsalException exc) {
      ...
      if (exc instanceof MsalIntuneAppProtectionPolicyRequiredException) {
        MsalIntuneAppProtectionPolicyRequiredException appException =
            (MsalIntuneAppProtectionPolicyRequiredException) exc;
